function [intersect, t, u, v, xcoor] = TriangleRayIntersection (...
  orig, dir, vert1, vert2, vert3, varargin)
...
if (size(vert3,1)==3 && size(vert3,2)~=3), vert3=vert3'; end

%% In case of single points clone them to the same size as the rest
N = max([size(orig,1), size(dir,1), size(vert1,1), size(vert2,1), size(vert3,1)]);
...
if (size(vert3,1)==1 && N>1 && size(vert3,2)==3), vert3 = repmat(vert3, N, 1); end

%% Check if all the sizes match
SameSize = (any(size(orig)==size(vert1)) && ...
  any(size(orig)==size(vert2)) && ...
  any(size(orig)==size(vert3)) && ...
...
edge1 = vert2-vert1;          % find vectors for two edges sharing vert0
edge2 = vert3-vert1;
tvec  = orig -vert1;          % vector from vert0 to ray origin
