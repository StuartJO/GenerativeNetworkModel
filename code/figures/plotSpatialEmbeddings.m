...
% This function plots the CDF and 
...
my_subplot(2,6,scatterplotInd);
    scatter(Data1,Data2,20,lines_cmap_alpha{i}(128,:),'filled')
    
    xlabel(['Mean empirical ',lowercasename])
    ylabel(['Mean model ',lowercasename])
    ...
    set(gca,'FontSize',12)
    text(text_loc_x,text_loc_y,['\fontsize{16}{\rho}\fontsize{12} = ',num2str(round(c,3)),', {\itp} = ',num2str(round(pVal,3))],'HorizontalAlignment','center')
    ...
    if ~isempty(SurfaceData.MissingROI)
        ...
        %Data1_allrois = [Data1(1:SurfaceData.MissingROI-1) NaN Data1(SurfaceData.MissingROI:end)];
        %Data2_allrois = [Data2(1:SurfaceData.MissingROI-1) NaN Data2(SurfaceData.MissingROI:end)];
    else
        Data1_allrois = Data1;
        Data2_allrois = Data2;
    end
