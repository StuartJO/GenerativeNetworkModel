function PlotPropSurf2(Emp,Model,Model_name,Cbar_title,plotLabel,cmap)
...
% lines_cmap = lines(4);
% 
% cmap = make_alpha_cmap(lines_cmap(prop,:),256);

%cmap = flipud(make_cmap('orangered',256,30,0));
% 
% if prop == 1
%     Emp = sum(EmpNet);
%     Model = sum(ModelNet);    
...
%     cmap = flipud(make_cmap('orangered',256,30,0));
% elseif prop == 2
%     Emp = betweenness_bin(EmpNet);
%     Model = betweenness_bin(ModelNet);  
%     PropType = 'Betweenness';
% elseif prop == 3
%     Emp = clustering_coef_bu(EmpNet)';
%     Model = clustering_coef_bu(ModelNet)';    
%     PropType = 'Clustering';
%     cmap = cmocean('deep') ;
% elseif prop == 4
%     Emp = sum(D.*EmpNet)/2;
%     Model = sum(D.*ModelNet)/2;    
%     PropType = 'Mean distance';
% end
