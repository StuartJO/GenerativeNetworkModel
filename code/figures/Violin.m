classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            if length(data) > 10000 && length(unique(data)) < 250
                % If the data is highly stratified the plot will look
                % weird. This simply reduces the number of values the
                % ksdensity function will be calculated over, making it 
                % look less weird 
               [density, value] = ksdensity(data, unique(data)', 'bandwidth', args.Bandwidth);
            else
